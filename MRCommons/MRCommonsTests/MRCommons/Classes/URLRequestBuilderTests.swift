...
class URLRequestBuilderTests: XCTestCase {
    ...
    func testInitializingRequest_withInvalidURL_shouldThrow() {
        XCTAssertThrowsError(try URLRequestBuilder(withURL: URL(string: "")).build(), "Initializing URLRequestBuilder should throw") { error in
    ...
    func testBuildURLRequest_withouthCustomHeaders_shouldMathExpectedParams() {
        ...
        XCTAssertNoThrow(request = try URLRequestBuilder(withURL: URL(string: Constants.url)).setHTTPMethod(httpMethod).build(),
        ...
        let commonHeaders = URLRequestBuilder.commonHeaders(httpMethod: httpMethod)
        ...
        URLRequestBuilder.commonHeaders(httpMethod: httpMethod).forEach {
    ...
    func testBuildURLRequest_withCustomHeaders_shouldMathExpectedParams() {
        ...
        var request: URLRequest!
        let httpMethod = HTTPMethod.get
        XCTAssertNoThrow(request = try URLRequestBuilder(withURL: URL(string: Constants.url)).setHTTPMethod(httpMethod).setHeaders(customHeaders).build(),
                         "Initializing URLRequestBuilder should not throw")
        XCTAssertEqual(request.url?.absoluteString, Constants.url,
                       "URL should be \(Constants.url) but was \(String(describing: request.url?.absoluteString))")
        XCTAssertEqual(request.httpMethod, httpMethod.value,
    ...
    func testRequestBody_withPostHTTPMethodAndURLEncodedHeader_shouldBeParamsAsString() {
        ...
        var request: URLRequest!
        XCTAssertNoThrow(request = try URLRequestBuilder(withURL: URL(string: Constants.url)).setHTTPMethod(httpMethod).setHeaders(customHeaders).setParameters(parameters).build(),
                         "Initializing URLRequestBuilder should not throw")
        XCTAssertEqual(request.url?.absoluteString, Constants.url,
                       "URL should be \(Constants.url) but was \(String(describing: request.url?.absoluteString))")
        XCTAssertEqual(request.httpMethod, httpMethod.value,
    ...
    func testRequestBody_withPostHTTPMethodAndJSONHeader_shouldBeParamsAsData() {
        ...
        let httpMethod = HTTPMethod.post
        var request: URLRequest!
        XCTAssertNoThrow(request = try URLRequestBuilder(withURL: URL(string: Constants.url)).setHTTPMethod(httpMethod).setParameters(parameters).setHeaders(customHeaders).build(),
                         "Initializing URLRequestBuilder should not throw")
        XCTAssertEqual(request.url?.absoluteString, Constants.url,
                       "URL should be \(Constants.url) but was \(String(describing: request.url?.absoluteString))")
        XCTAssertEqual(request.httpMethod, httpMethod.value,
    ...
    func testRequestBody_withParameters_shouldBeAppendedToURL() {
        ...
        let httpMethod = HTTPMethod.get
        var request: URLRequest!
        XCTAssertNoThrow(request = try URLRequestBuilder(withURL: URL(string: Constants.url)).setParameters(parameters).build(),
                         "Initializing URLRequestBuilder should not throw")
        XCTAssertEqual(request.httpMethod, httpMethod.value,
                       "HTTP method should be \(httpMethod.value) but was \(String(describing: request.httpMethod))")
    ...
    func testCreateURLRequestBuilder_withInvalidURL_shouldThrowInvalidURL() {
        XCTAssertThrowsError(try URLRequestBuilder(withURL: URL(string: "")).build(),
